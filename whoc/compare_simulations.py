...
def compare_simulations(results_dfs):
    ...
    for case_name, results_df in results_dfs.items():
        ...
        result_summary_dict["YawAngleChangeAbs"].append(results_df[[c for c in results_df.columns if "YawAngleChange" in c]].abs().sum().to_numpy().mean())
        result_summary_dict["FarmPowerSum"].append(results_df["FarmPower"].sum())
        result_summary_dict["FarmPower"].append(results_df["FarmPower"].mean())
