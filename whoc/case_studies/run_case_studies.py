...
from whoc.case_studies.initialize_case_studies import (
    initialize_simulations,
    case_families,
    case_studies,
)
from whoc.case_studies.simulate_case_studies import simulate_controller
from whoc.case_studies.process_case_studies import (
    read_time_series_data,
    write_case_family_time_series_data,
    read_case_family_time_series_data,
    aggregate_time_series_data,
    read_case_family_agg_data,
    write_case_family_agg_data,
    generate_outputs,
    plot_simulations,
    plot_wind_farm,
    plot_breakdown_robustness,
    plot_horizon_length,
    plot_cost_function_pareto_curve,
    plot_yaw_offset_wind_direction,
    plot_parameter_sweep,
    plot_power_increase_vs_prediction_time,
    plot_power_vs_prediction_time,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="run_case_studies.py", description="Run FLORIS case studies for WHOC module."
    )
    parser.add_argument(
        "case_ids", metavar="C", nargs="+", choices=[str(i) for i in range(len(case_families))]
    )
    ...
    parser.add_argument(
        "-sd", "--save_dir", type=str, default=os.path.join(os.getcwd(), "simulation_results")
    )
    ...
    RUN_ONCE = (
        (args.multiprocessor == "mpi" and (comm_rank := comm.Get_rank()) == 0)
    ...
    if RUN_ONCE:
        ...
        logging.info(
            f"running initialize_simulations for case_ids {[case_families[i] for i in args.case_ids]}"
        )
        input_dicts, wind_field_config, wind_field_ts = initialize_simulations(
            case_study_keys=[case_families[i] for i in args.case_ids],
    ...
    if "CUDA_VISIBLE_DEVICES" in os.environ:
        ...
        try:
            ...
            if num_visible_gpus > 0:
                ...
                logging.info(
                    f"Found {num_visible_gpus} GPUs. Setting max_workers to num_visible_gpus={max_workers}."
                )
            else:
                max_workers = comm.Get_size() if args.multiprocessor == "mpi" else mp.cpu_count()
                logging.warning(
    ...
    if args.run_simulations:
        if args.multiprocessor is not None:
            ...
            with executor as run_simulations_exec:
                ...
                logging.info(
                    f"Submitting simulate_controller calls to pool executor with {run_simulations_exec._max_workers} workers"
                )
                ...
                futures = [
                    run_simulations_exec.submit(
                        ...
                        wind_forecast_class=globals()[d["controller"]["wind_forecast_class"]]
                        if d["controller"]["wind_forecast_class"]
                        else None,
    ...
    if args.postprocess_simulations:
        ...
        if (
            ...
            or not all(
                os.path.exists(os.path.join(args.save_dir, case_families[i], "agg_results_all.csv"))
                for i in args.case_ids
            )
        ):
            # if RUN_ONCE:
            ...
            if args.multiprocessor is not None:
                ...
                with executor as run_simulations_exec:
                    ...
                    read_futures = [
                        ...
                        if args.reaggregate_simulations
                    ...
                    read_futures = [
                        ...
                        for i in args.case_ids
                        if not args.reaggregate_simulations
                        and os.path.exists(
                            os.path.join(
                                args.save_dir, case_families[i], "time_series_results_all.csv"
                            )
            ...
            # else, run sequentially
                ...
                for i in args.case_ids:
                    ...
                    if not args.reaggregate_simulations and os.path.exists(
                        os.path.join(args.save_dir, case_families[i], "time_series_results_all.csv")
                    ):
                        existing_time_series_df.append(
                            read_case_family_time_series_data(
                                case_families[i], save_dir=args.save_dir
                            )
                        )
            ...
            if RUN_ONCE:
                ...
                unique_seeds = (
                    time_series_df.groupby(["CaseFamily", "CaseName"], level=0)["WindSeed"]
                    .unique()
                    .values
                )
                ...
                if args.reaggregate_simulations or not all(
                    os.path.exists(
                        os.path.join(args.save_dir, case_families[i], "agg_results_all.csv")
                    )
                    for i in args.case_ids
                ):
                    ...
                    min_stop_time_per_seed = (
                        time_series_df.groupby(
                            ["CaseFamily", "CaseName", "WindSeed"], group_keys=False
                        )["Time"]
                        .max()
                        .groupby(["WindSeed"], group_keys=False)
                        .min()
                    )

                    for i in args.case_ids:
                        # for case_name in set([re.findall(r"(?<=case_)(.*)(?=_seed)", fn)[0] for fn in case_family_case_names[case_families[i]]]):
                        case_family_df = time_series_df.iloc[
                            time_series_df.index.get_level_values("CaseFamily") == case_families[i],
                            :,
                        ]
                        case_desc = pd.read_csv(
                            os.path.join(args.save_dir, case_families[i], "case_descriptions.csv"),
                            index_col=0,
                        )
                    ...
                    trunc_time_series_df = trunc_time_series_df.apply(
                        func=(
                            lambda sub_df: sub_df[
                                sub_df["Time"].between(
                                    max_ctx_time,
                                    min_stop_time_per_seed.iloc[sub_df.name[2]],
                                    inclusive="both",
                                )
                            ]
                        ),
                    ...
                    trunc_time_series_df = trunc_time_series_df.reset_index(
                        level="WindSeed", drop=False
                    )
                    trunc_time_series_df = trunc_time_series_df[
                        trunc_time_series_df["WindSeed"].isin(common_seeds)
                    ]
                ...
                for i in args.case_ids:
                    if not args.reaggregate_simulations and os.path.exists(
                        ...
                        existing_agg_df.append(
                            read_case_family_agg_data(case_families[i], save_dir=args.save_dir)
                        )
                ...
                agg_df.index = pd.MultiIndex.from_frame(
                    agg_df.index.to_frame().astype({"CaseFamily": str, "CaseName": str})
                )
            ...
            for i in args.case_ids:
                ...
                if os.path.exists(filepath):
                    ...
                    bool_columns = [
                        col for col in columns if any(c in col for c in ["TurbineOfflineStatus"])
                    ]
            ...
            time_series_df = (
                pl.concat(time_series_df, how="vertical")
                .to_pandas()
                .set_index(["CaseFamily", "CaseName"])
            )
            ...
            for i in args.case_ids:
                ...
                if os.path.exists(filepath):
                    ...
                    df = pd.read_csv(
                        ...
                        dtype={
                            **{"CaseFamily": str, "CaseName": str},
                            **{col: float for col in numeric_columns},
                            **{col: bool for col in bool_columns},
                        },
                    )
                    df.index = pd.MultiIndex.from_frame(
                        df.index.to_frame().astype({"CaseFamily": str, "CaseName": str})
                    )
        ...
        if RUN_ONCE and PLOT:
            ...
            if (
                any(
                    case_families.index(cf) in args.case_ids
                    for cf in [
                ...
                and (
                    case_families.index("baseline_controllers_baseline_det_forecasters_awaken")
                    in args.case_ids
                )
                and (
                ...
                baseline_time_df = time_series_df.loc[
                    time_series_df.index.get_level_values("CaseFamily").isin(cfs), :
                ]  # .reset_index(level="CaseFamily", drop=True)
                baseline_agg_df = agg_df.loc[
                    agg_df.index.get_level_values("CaseFamily").isin(cfs), :
                ]  # .reset_index(level="CaseFamily", drop=True)
                ...
                baseline_det_case_desc = pd.read_csv(
                    os.path.join(
                        args.save_dir,
                        "baseline_controllers_baseline_det_forecasters_awaken",
                        "case_descriptions.csv",
                ...
                base_val_idx = perfect_case_desc.loc[
                    pd.to_timedelta(perfect_case_desc["prediction_timedelta"]).isin(
                ...
                base_cond = (
                    (
                        baseline_time_df.index.get_level_values("CaseFamily")
                        == "baseline_controllers_perfect_forecaster_awaken"
                    )
                ...
                base_cond = (
                    (
                        baseline_agg_df.index.get_level_values("CaseFamily")
                        == "baseline_controllers_perfect_forecaster_awaken"
                    )
                ...
                for (case_family, case_name), _ in baseline_agg_df.iterrows():
                    ...
                    for col in config_cols:
                        baseline_time_df.loc[
                            (baseline_time_df.index.get_level_values("CaseFamily") == case_family)
                            & (baseline_time_df.index.get_level_values("CaseName") == case_name),
                            col,
                        ] = full_config[col]
                        baseline_agg_df.loc[
                            (baseline_agg_df.index.get_level_values("CaseFamily") == case_family)
                            & (baseline_agg_df.index.get_level_values("CaseName") == case_name),
                            col,
                        ] = full_config[col]

                ml_cond = baseline_agg_df["wind_forecast_class"] == "MLForecast"
                baseline_agg_df.loc[ml_cond, "wind_forecast_class"] = (
                    baseline_agg_df.loc[ml_cond, "model_key"].str.capitalize() + "Forecast"
                ).values

                # Filter data for the two forecast types
                forecasters_agg_df = baseline_agg_df.loc[
                    baseline_agg_df["wind_forecast_class"] != "PerfectForecast", :
                ]
                perfect_agg_df = baseline_agg_df.loc[
                    (baseline_agg_df["wind_forecast_class"] == "PerfectForecast")
                    & baseline_agg_df.index.get_level_values("CaseFamily").str.contains(
                        "baseline_controllers_baseline_det_forecasters"
                    ),
                    :,
                ]
                ...
                ml_baseline_agg_df = baseline_agg_df.loc[
                    (~baseline_agg_df["model_key"].isnull())
                    | (
                        (baseline_agg_df["wind_forecast_class"] == "PerfectForecast")
                ...
                ml_baseline_agg_df["controller_class"] = ml_baseline_agg_df[
                    "controller_class"
                ] + ml_baseline_agg_df["uncertain"].astype(str)
                ...
                other_baseline_agg_df = baseline_agg_df.loc[
                    baseline_agg_df["model_key"].isnull(), :
                ]
                other_baseline_agg_df["controller_class"] = other_baseline_agg_df[
                    "controller_class"
                ] + other_baseline_agg_df["uncertain"].astype(str)
                ...
                best_forecaster_prediction_delta = forecasters_agg_df.groupby(
                    "wind_forecast_class", group_keys=False
                ).apply(
                    lambda x: x.sort_values(by=("FarmPower", "mean"), ascending=False).head(10),
                    include_groups=False,
                )  # [("FarmPower", "mean")]
                best_perfect_prediction_delta = perfect_agg_df.groupby(
                    "wind_forecast_class", group_keys=False
                ).apply(
                    lambda x: x.sort_values(by=("FarmPower", "mean"), ascending=False).head(10),
                    include_groups=False,
                ...
                forecasters_agg_df.groupby("prediction_timedelta", group_keys=False).apply(
                    lambda x: x.sort_values(by=("FarmPower", "mean"), ascending=False).head(10),
                    include_groups=False,
                )  # [("FarmPower", "mean")]

                # plot forecasters, persistent, perfect for 60/300sec predictions
                perfect_case_names = perfect_agg_df.loc[
                    perfect_agg_df["prediction_timedelta"].isin(
                        pd.unique(forecasters_agg_df["prediction_timedelta"])
                    )
                ].index.get_level_values("CaseName")
                persistence_case_names = baseline_agg_df.loc[
                    (baseline_agg_df["wind_forecast_class"] == "PerfectForecast")
                    & baseline_agg_df.index.get_level_values("CaseFamily").str.contains(
                        "baseline_controllers_perfect_forecaster_awaken"
                    ),
                ...
                plotting_cases = (
                    [(df[1]._name[0], df[1]._name[1]) for df in forecasters_agg_df.iterrows()]
                    + [
                        ("baseline_controllers_baseline_det_forecasters_awaken", cn)
                        for cn in perfect_case_names
                    ]
                    + [
                        ("baseline_controllers_perfect_forecaster_awaken", cn)
            ...
            if (
                case_families.index("baseline_controllers_perfect_forecaster_awaken")
                in args.case_ids
                or case_families.index("baseline_controllers_baseline_perfect0_forecasters_awaken")
                in args.case_ids
                or case_families.index("baseline_controllers_perfect_forecaster_flasc")
                in args.case_ids
                or case_families.index("baseline_controllers_forecasters_test_awaken")
                in args.case_ids
                or case_families.index("baseline_controllers_informer_forecaster_test_awaken")
                in args.case_ids
                or case_families.index("baseline_controllers_svr_forecaster_test_awaken")
                in args.case_ids
            ):
                ...
                x = (
                    ...
                    .sort_values(("FarmPower", "mean"), ascending=False)
                )
                x[
                    [
                        ("FarmPower", "mean"),
                ...
                if False:
                    ...
                    time_series_df = pd.merge(
                        time_series_df, extra_args, on=["CaseFamily", "CaseName"]
                    )
                    x = (
                        time_series_df.reset_index(drop=True)[
                            [
                                "controller_class",
                                "prediction_timedelta",
                                "FarmPower",
                        ...
                        .set_index(["Time", "controller_class", "WindSeed"])
                        .sort_values(["controller_class", "Time"])
                    )

                    # get lowest end time available
                    end_times_per_seed = (
                        x.groupby(["WindSeed", "prediction_timedelta"])
                        .apply(lambda x: x.sort_values("Time", ascending=True).tail(1))
                        .reset_index(level=[0, 1], drop=True)
                        .reset_index(0, drop=False)[["Time", "prediction_timedelta"]]
                        .groupby("WindSeed")
                        .agg("min")["Time"]
                    )
                    x = x.groupby("WindSeed", group_keys=False).apply(
                        func=(
                            lambda x: x.loc[
                                x.index.get_level_values("Time")
                                <= end_times_per_seed[x.index.get_level_values("WindSeed")[0]]
                            ]
                        )
                    )
                    x = (
                        x[["prediction_timedelta", "FarmPower"]]
                        .groupby(["controller_class", "prediction_timedelta", "WindSeed"])
                        .agg("mean")
                        .reset_index("prediction_timedelta")
                    )
                    ...
                    x.loc[x["FarmPower_x"] > 0.5, :].groupby(
                        "controller_class", group_keys=False
                    ).apply(lambda x: x.sort_values("FarmPower_x", ascending=False))[
                        ["prediction_timedelta_x", "FarmPower_x"]
                    ]  # .to_csv("/Users/ahenry/Desktop/perfect.csv")

                    # find % increase in farm power for each controller class and Wind Seed, averaged over all prediction_timedelta_values
                    x.groupby(["controller_class", "WindSeed"])["FarmPower_x"].agg("mean").groupby(
                        "controller_class", group_keys=False
                    ...
                    x.groupby(["controller_class", "prediction_timedelta_x"])["FarmPower_x"].agg(
                        "mean"
                    ).groupby("controller_class", group_keys=False).apply(
                ...
                if (
                    "use_upstream_wind" in perfect_agg_df.columns
                    and "filter_floris_wind" in perfect_agg_df.columns
                ):
                    perfect_agg_df.groupby(
                        ["use_upstream_wind", "filter_floris_wind", "controller_class"]
                    )["FarmPower"].agg("mean")[("FarmPower", "mean")].groupby(
                        "controller_class", group_keys=False
                    ).apply(lambda x: x.sort_values(ascending=False))
            ...
            if case_families.index("baseline_controllers") in args.case_ids:
                mpc_df = agg_df.iloc[
                    agg_df.index.get_level_values("CaseFamily") != "baseline_controllers"
                ]
                lut_df = agg_df.iloc[
                    (agg_df.index.get_level_values("CaseFamily") == "baseline_controllers")
                    & (agg_df.index.get_level_values("CaseName") == "LUT")
                ]
                greedy_df = agg_df.iloc[
                    (agg_df.index.get_level_values("CaseFamily") == "baseline_controllers")
                    & (agg_df.index.get_level_values("CaseName") == "Greedy")
                ]

                better_than_lut_df = (
                    mpc_df.loc[
                        (mpc_df[("FarmPower", "mean")] > lut_df[("FarmPower", "mean")].iloc[0])
                        & (
                            mpc_df[("YawAngleChangeAbs", "mean")]
                            < lut_df[("YawAngleChangeAbs", "mean")].iloc[0]
                        ),
                        [
                            ("RelativeTotalRunningOptimizationCostMean", "mean"),
                            ("YawAngleChangeAbs", "mean"),
                            ("FarmPower", "mean"),
                        ],
                    ]
                    .sort_values(
                        by=("RelativeTotalRunningOptimizationCostMean", "mean"), ascending=True
                    )
                    .reset_index(level="CaseFamily", drop=True)
                )
                better_than_greedy_df = (
                    mpc_df.loc[
                        (mpc_df[("FarmPower", "mean")] > greedy_df[("FarmPower", "mean")].iloc[0]),
                        [
                            ("RelativeTotalRunningOptimizationCostMean", "mean"),
                            ("YawAngleChangeAbs", "mean"),
                            ("FarmPower", "mean"),
                ...
                100 * (
                    better_than_lut_df.iloc[0]["FarmPower"] - lut_df.iloc[0]["FarmPower"]
                ) / lut_df.iloc[0]["FarmPower"]
                100 * (
                    better_than_lut_df.iloc[0]["FarmPower"] - greedy_df.iloc[0]["FarmPower"]
                ) / greedy_df.iloc[0]["FarmPower"]
                100 * (
                    better_than_lut_df.iloc[0]["YawAngleChangeAbs"]
                    - lut_df.iloc[0]["YawAngleChangeAbs"]
                ) / lut_df.iloc[0]["YawAngleChangeAbs"]
                100 * (
                    better_than_lut_df.iloc[0]["YawAngleChangeAbs"]
                    - greedy_df.iloc[0]["YawAngleChangeAbs"]
                ) / greedy_df.iloc[0]["YawAngleChangeAbs"]

                if True:
                    plotting_cases = [
                        ("wind_preview_type", better_than_lut_df.iloc[0]._name),
            ...
            if (case_families.index("baseline_controllers") in args.case_ids) and (
                case_families.index("cost_func_tuning") in args.case_ids
            ):
                mpc_alpha_df = agg_df.iloc[
                    agg_df.index.get_level_values("CaseFamily") == "cost_func_tuning"
                ]
                ...
                if case_families.index("baseline_controllers") in args.case_ids:
                    ...
                    lut_df = agg_df.iloc[
                        (agg_df.index.get_level_values("CaseFamily") == "baseline_controllers_3")
                        & (agg_df.index.get_level_values("CaseName") == "LUT")
                    ]
                    greedy_df = agg_df.iloc[
                        (agg_df.index.get_level_values("CaseFamily") == "baseline_controllers_3")
                ...
                mpc_alpha_df[
                    [
                        ("RelativeTotalRunningOptimizationCostMean", "mean"),
                        ("RelativeRunningOptimizationCostTerm_0", "mean"),
                        ("RelativeRunningOptimizationCostTerm_1", "mean"),
                        ("YawAngleChangeAbs", "mean"),
                        ("FarmPower", "mean"),
                    ]
                ].sort_values(by=("FarmPower", "mean"), ascending=False).reset_index(
                    level="CaseFamily", drop=True
                )
                ...
                better_than_lut_df = mpc_alpha_df.loc[
                    (mpc_alpha_df[("FarmPower", "mean")] > lut_df[("FarmPower", "mean")].iloc[0])
                    & (
                        mpc_alpha_df[("YawAngleChangeAbs", "mean")]
                        < lut_df[("YawAngleChangeAbs", "mean")].iloc[0]
                    ),
                    [
                        ("RelativeTotalRunningOptimizationCostMean", "mean"),
                ...
                better_than_greedy_df = (
                    mpc_alpha_df.loc[
                        (
                            mpc_alpha_df[("FarmPower", "mean")]
                            > greedy_df[("FarmPower", "mean")].iloc[0]
                        ),
                        [
                            ("RelativeTotalRunningOptimizationCostMean", "mean"),
                            ("YawAngleChangeAbs", "mean"),
                ...
                better_than_lut_df = better_than_lut_df.sort_values(
                    by=("FarmPower", "mean"), ascending=False
                )
                100 * (
                    better_than_lut_df.iloc[0]["FarmPower"] - lut_df.iloc[0]["FarmPower"]
                ) / lut_df.iloc[0]["FarmPower"]
                ...
                better_than_lut_df = better_than_lut_df.sort_values(
                    by=("YawAngleChangeAbs", "mean"), ascending=True
                )
                100 * (
                    better_than_lut_df.iloc[0]["YawAngleChangeAbs"]
                    - lut_df.iloc[0]["YawAngleChangeAbs"]
                ...
                100 * (
                    lut_df.iloc[0]["FarmPower"] - greedy_df.iloc[0]["FarmPower"]
                ) / greedy_df.iloc[0]["FarmPower"]
                100 * (
                    lut_df.iloc[0]["YawAngleChangeAbs"] - greedy_df.iloc[0]["YawAngleChangeAbs"]
                ) / greedy_df.iloc[0]["YawAngleChangeAbs"]

                plot_cost_function_pareto_curve(agg_df, args.save_dir)

            if (case_families.index("baseline_controllers") in args.case_ids) and (
                case_families.index("scalability") in args.case_ids
            ):
            ...
            if (case_families.index("baseline_controllers") in args.case_ids) and (
                case_families.index("horizon_length") in args.case_ids
            ):
                mpc_df = agg_df.iloc[
                    agg_df.index.get_level_values("CaseFamily") == "horizon_length"
                ][
                    [
                        ("YawAngleChangeAbs", "mean"),
                        ("FarmPower", "mean"),
                        ("OptimizationConvergenceTime", "mean"),
                    ]
                ].sort_values(
                    by=("FarmPower", "mean"), ascending=False
                )  # .reset_index(level="CaseFamily", drop=True)
                ...
                for (case_family, case_name), _ in mpc_df.iterrows():
                    ...
                    for col in config_cols:
                        mpc_df.loc[
                            ...
                            col,
                        ] = input_config["controller"][col]

                lut_df = agg_df.iloc[
                    agg_df.index.get_level_values("CaseFamily").str.contains("baseline_controllers")
                    & (agg_df.index.get_level_values("CaseName") == "LUT")
                ][
                    [
                        ("YawAngleChangeAbs", "mean"),
            ...
            if case_families.index("yaw_offset_study") in args.case_ids:
                mpc_alpha_df = agg_df.iloc[
                    (agg_df.index.get_level_values("CaseFamily") == "yaw_offset_study")
                    & (~agg_df.index.get_level_values("CaseName").str.contains("LUT"))
                ]
                lut_df = agg_df.iloc[
                    (agg_df.index.get_level_values("CaseFamily") == "yaw_offset_study")
                    & (agg_df.index.get_level_values("CaseName").str.contains("LUT"))
                ...
                if "baseline_controllers_3" in agg_df.index.get_level_values("CaseFamily"):
                    greedy_df = agg_df.iloc[
                        (agg_df.index.get_level_values("CaseFamily") == "baseline_controllers_3")
                        & (agg_df.index.get_level_values("CaseName").str.contains("Greedy"))
                    ]
                    ...
                    plotting_cases = [
                        ("yaw_offset_study", better_than_lut_df.iloc[0]._name),
                ...
                case_names = [
                    "LUT_3turb",
                    "StochasticIntervalRectangular_1_3turb",
                    "StochasticIntervalRectangular_11_3turb",
                    "StochasticIntervalElliptical_11_3turb",
                    "StochasticSample_25_3turb",
                    "StochasticSample_100_3turb",
                ]
                case_labels = [
                    "LUT",
                    "MPC\n1 RI Samples",
                ...
                plot_yaw_offset_wind_direction(
                    ...
                    case_labels,
                    os.path.join(
                        os.path.dirname(whoc.__file__),
                        f"../examples/mpc_wake_steering_florisstandin/lookup_tables/lut_{3}.csv",
                    ),
                    os.path.join(args.save_dir, "yaw_offset_study", "yawoffset_winddir_ts.png"),
                    plot_turbine_ids=[0, 1, 2],
                    include_yaw=True,
                    include_power=True,
                    scatter=False,
                )

                for sub_case_names, sub_case_labels, filename in zip(
                    [
                        ["LUT_3turb"],
                        [
                            "StochasticIntervalRectangular_1_3turb",
            ...
            if (case_families.index("baseline_controllers_3") in args.case_ids) and (
                case_families.index("gradient_type") in args.case_ids
                or case_families.index("n_wind_preview_samples") in args.case_ids
            ):
                ...
                mpc_df["diff_direction"] = mpc_df["diff_type"].apply(
                    lambda s: s.split("_")[1] if s != "none" else None
                )
                mpc_df["diff_steps"] = mpc_df["diff_type"].apply(
                    lambda s: s.split("_")[0] if s != "none" else None
                )
                mpc_df["n_wind_preview_samples_index"] = None

                unique_sir_n_samples = np.sort(
                    pd.unique(
                        mpc_df.loc[
                ...
                unique_ss_n_samples = np.sort(
                    pd.unique(
                        mpc_df.loc[
                            mpc_df["wind_preview_type"] == "stochastic_sample",
                            "n_wind_preview_samples",
                        ]
                    )
                )
                mpc_df.loc[
                    ...
                    .apply(lambda n: np.where(unique_sir_n_samples == n)[0][0])
                    .astype("int")
                    .values
                )
                mpc_df.loc[
                    ...
                    .apply(lambda n: np.where(unique_sie_n_samples == n)[0][0])
                    .astype("int")
                    .values
                )
                mpc_df.loc[
                    ...
                    .apply(lambda n: np.where(unique_ss_n_samples == n)[0][0])
                    .astype("int")
                    .values
                )
                ...
                better_than_lut_df = (
                    mpc_df.loc[
                        (mpc_df["FarmPower"] > lut_df["FarmPower"].iloc[0])
                        & (mpc_df["YawAngleChangeAbs"] < lut_df["YawAngleChangeAbs"].iloc[0]),
                        ["YawAngleChangeAbs", "OptimizationConvergenceTime", "FarmPower"]
                        + config_cols,
                    ]
                    .sort_values(by="FarmPower", ascending=False)
                    .reset_index(level="CaseFamily", drop=True)
                )
                ...
                better_than_lut_df.drop(["n_wind_preview_samples", "n_horizon"], axis=1).loc[
                    better_than_lut_df.index.get_level_values("CaseName").isin(
                ...
                for param in ["diff_type", "decay_type", "max_std_dev", "nu"]:
                    for agg_type in ["mean", "max"]:
                        ...
                        logging.info(
                            better_than_lut_df.drop(["n_wind_preview_samples", "n_horizon"], axis=1)
                            .groupby(["wind_preview_type", param])["FarmPower"]
                            .agg(agg_type)
                            .groupby("wind_preview_type")
                            .idxmax()
                            .values
                        )
                ...
                if True:
                    plot_parameter_sweep(
                        pd.concat([mpc_df, lut_df, greedy_df]),
                        MPC_TYPE,
                        args.save_dir,
                        plot_columns=[
                            "FarmPower",
                ...
                plotting_cases = [
                    (
                        MPC_TYPE,
                        better_than_lut_df.sort_values(by="FarmPower", ascending=False)
                        .iloc[0]
                        ._name,
                    ),
            ...
            if case_families.index("wind_preview_type") in args.case_ids:
                ...
                wind_preview_type_df = (
                    agg_df.iloc[agg_df.index.get_level_values("CaseFamily") == "wind_preview_type"][
                        [
                            ("RelativeTotalRunningOptimizationCostMean", "mean"),
                            ("YawAngleChangeAbs", "mean"),
                            ("FarmPower", "mean"),
            ...
            if (case_families.index("baseline_controllers") in args.case_ids) and (
                ...
                mpc_df = agg_df.iloc[
                    agg_df.index.get_level_values("CaseFamily") == "gradient_type", :
                ]
                lut_df = agg_df.iloc[
                    (agg_df.index.get_level_values("CaseFamily") == "baseline_controllers")
                    & (agg_df.index.get_level_values("CaseName") == "LUT")
            ...
            if all(
                case_families.index(cf) in args.case_ids
                for cf in ["baseline_controllers", "solver_type", "wind_preview_type", "warm_start"]
            ):
                generate_outputs(agg_df, args.save_dir)

            if (
                case_families.index("baseline_controllers_perfect_forecaster_flasc")
                in args.case_ids
                or case_families.index("baseline_controllers_perfect_forecaster_awaken")
                in args.case_ids
            ):
                if (
                    case_families.index("baseline_controllers_preview_flasc_perfect")
                    in args.case_ids
                ):
                    mpc_df = agg_df.loc[
                        agg_df.index.get_level_values("CaseFamily")
                ...
                if (
                    "prediction_timedelta" in forecasters_df.columns
                    and "prediction_timedelta" in perfect_df.columns
                ):
                ...
                merged_df["power_ratio"] = (
                    merged_df["FarmPowerMean_kalman", "mean"]
                    / merged_df["FarmPowerMean_perfect", "mean"]
                ) * 100
