...
def simulate_controller(controller_class, wind_forecast_class, simulation_input_dict, **kwargs):
    ...
    if (
        simulation_input_dict["controller"]["uncertain"]
        and not wind_forecast_class.is_probabilistic
    ):
        logging.info(
            f"Can't run with uncertain flag for {wind_forecast_class.__name__}, setting uncertainty off."
        )
        simulation_input_dict["controller"]["uncertain"] = (
            simulation_input_dict["controller"]["uncertain"]
            and wind_forecast_class.is_probabilistic
        )
    ...
    # Load a FLORIS object for power calculations
    fi = ControlledFlorisModel(
        t0=kwargs["wind_field_ts"].select(pl.col("time").first()).item(),
        ...
        target_turbine_indices=simulation_input_dict["controller"]["target_turbine_indices"]
        or "all",
    ...
    stoptime = (
        simulation_input_dict["hercules_comms"]["helics"]["config"]["stoptime"]
        - simulation_input_dict["wind_forecast"]["prediction_timedelta"].total_seconds()
        - (
            simulation_input_dict["controller"]["n_horizon"]
            * simulation_input_dict["controller"]["controller_dt"]
        )
    )  # - 2*TRUNCATE_STEPS

    load_from_checkpoint = (
        not kwargs["rerun_simulations"]
        and os.path.exists(temp_save_path)
        and not kwargs["skip_temps"]
    )
    ...
    if load_from_final:
        ...
        for offset in range(df_len - 100, -1, -100):
            chunk_df = (
                ...
                .select(
                    ["Time", "FreestreamWindMag"]
                    + [f"TurbineYawAngle_{idx2tid_mapping[i]}" for i in fi.sorted_tids]
                )
                .slice(offset, 100)
                .drop_nulls(subset="FreestreamWindMag")
                .tail(1)
                .collect()
            )
            if chunk_df.shape[0] == 0:
                ...
                simulation_input_dict["controller"]["initial_conditions"]["yaw"] = (
                    chunk_df[[f"TurbineYawAngle_{idx2tid_mapping[i]}" for i in fi.sorted_tids]]
                    .to_numpy()
                    .flatten()
                )
                break
    ...
    logging.info(
        f"Running instance of {controller_class.__name__} - {kwargs['case_name']} with wind seed {kwargs['wind_case_idx']}"
    )

    kwargs["wind_field_config"]["preview_dt"] = int(
        simulation_input_dict["controller"]["controller_dt"]
        / simulation_input_dict["simulation_dt"]
    )
    kwargs["wind_field_config"]["n_preview_steps"] = simulation_input_dict["controller"][
        "n_horizon"
    ] * int(
        simulation_input_dict["controller"]["controller_dt"]
        / simulation_input_dict["simulation_dt"]
    )
    kwargs["wind_field_config"]["time_series_dt"] = int(
        simulation_input_dict["controller"]["controller_dt"]
        // simulation_input_dict["simulation_dt"]
    )
    ...
    if (
        isinstance(simulation_input_dict["controller"]["initial_conditions"]["yaw"], str)
        and simulation_input_dict["controller"]["initial_conditions"]["yaw"] == "auto"
    ):
        if "FreestreamWindDir" in kwargs["wind_field_ts"].columns:
            simulation_input_dict["controller"]["initial_conditions"]["yaw"] = [
                kwargs["wind_field_ts"].select(pl.col("FreestreamWindDir").first()).item()
            ] * fi.n_turbines
        else:
            u = (
                kwargs["wind_field_ts"]
                .select([f"ws_horz_{idx2tid_mapping[t_idx]}" for t_idx in fi.sorted_tids])
                .slice(0, 1)
                .to_numpy()[0, :]
            )
            v = (
                kwargs["wind_field_ts"]
                .select([f"ws_vert_{idx2tid_mapping[t_idx]}" for t_idx in fi.sorted_tids])
                .slice(0, 1)
                .to_numpy()[0, :]
            )
            simulation_input_dict["controller"]["initial_conditions"]["yaw"] = 180.0 + np.rad2deg(
                np.arctan2(u, v)
            )
    ...
    if kwargs["wf_source"] == "scada":
        if simulation_input_dict["controller"]["target_turbine_indices"] == "all":
            simulation_u = (
                kwargs["wind_field_ts"]
                .select([f"ws_horz_{idx2tid_mapping[t_idx]}" for t_idx in idx2tid_mapping])
                .select(pl.mean_horizontal(pl.all()))
            )
            simulation_v = (
                kwargs["wind_field_ts"]
                .select([f"ws_vert_{idx2tid_mapping[t_idx]}" for t_idx in idx2tid_mapping])
                .select(pl.mean_horizontal(pl.all()))
            )
        else:
            ...
            if use_upstream_wind:
                    ...
                    + np.rad2deg(
                        np.arctan2(
                            (
                                ...
                                .select(
                                    [
                                        f"ws_horz_{idx2tid_mapping[t_idx]}"
                                        for t_idx in fi.sorted_tids
                                    ]
                                )
                                .select(pl.mean_horizontal(pl.all()))
                                .to_numpy()
                            ...
                            (
                                ...
                                .select(
                                    [
                                        f"ws_vert_{idx2tid_mapping[t_idx]}"
                                        for t_idx in fi.sorted_tids
                                    ]
                                )
                                .select(pl.mean_horizontal(pl.all()))
                ...
                wd[wd < 0] = 360.0 + wd[wd < 0]
                wd[wd > 360] = np.mod(wd[wd > 360], 360.0)
                ...
                del layout_x_rot, wd
                upstream_turbine_id = [
                    idx2tid_mapping[fi.sorted_tids[t_idx]] for t_idx in upstream_turbine_idx
                ]
                logging.info(
                    f"Using turbine id {np.unique(upstream_turbine_id, return_counts=False)} as upstream turbine for wind seed {kwargs['wind_case_idx']}."
                )
                del upstream_turbine_id
                simulation_u = (
                    kwargs["wind_field_ts"]
                    .select([f"ws_horz_{idx2tid_mapping[t_idx]}" for t_idx in fi.sorted_tids])
                    .to_numpy()[
                        np.arange(kwargs["wind_field_ts"].select(pl.len()).item()),
                        upstream_turbine_idx,
                    ]
                )
                simulation_v = (
                    kwargs["wind_field_ts"]
        ...
        if simulation_input_dict["controller"]["filter_floris_wind"]:
            ...
            fs_1 = (1 / (ts_len * simulation_input_dict["simulation_dt"])) * np.arange(1, half_len)
            fs_2 = 0.5 / simulation_input_dict["simulation_dt"]

            # tf_dir_lpf = butterworth_LPF_TFmag(fs, fc_dir, n_lpf)
            tf_mag_lpf_1 = butterworth_LPF_TFmag(fs_1, fc_mag, n_lpf)
            tf_mag_lpf_2 = butterworth_LPF_TFmag(fs_2, fc_mag, n_lpf)

            # Apply LPF magnitude
            # freq_vec_dir[1:int(ts_len / 2)] *= tf_dir_lpf
            freq_vec_u[1:half_len] *= tf_mag_lpf_1
            freq_vec_v[1:half_len] *= tf_mag_lpf_1

            if ts_len % 2 == 0:
                freq_vec_u[half_len] = np.sqrt(np.max([tf_mag_lpf_2, 0]))
                freq_vec_u[half_len + 1 :] *= np.flip(tf_mag_lpf_1)
                freq_vec_v[half_len] = np.sqrt(np.max([tf_mag_lpf_2, 0]))
                freq_vec_v[half_len + 1 :] *= np.flip(tf_mag_lpf_1)
            else:
                freq_vec_u[half_len : half_len + 2] = np.sqrt(np.max([tf_mag_lpf_2, 0]))
                freq_vec_u[half_len + 2 :] *= np.flip(tf_mag_lpf_1)
                freq_vec_v[half_len : half_len + 2] = np.sqrt(np.max([tf_mag_lpf_2, 0]))
                freq_vec_v[half_len + 2 :] *= np.flip(tf_mag_lpf_1)
        ...
        stoptime = (
            len(simulation_u) * simulation_input_dict["simulation_dt"]
            - int(simulation_input_dict["wind_forecast"]["prediction_timedelta"].total_seconds())
            - int(
                (
                    (
                        simulation_input_dict["controller"]["n_horizon"]
                        if controller_class.__name__ == "MPC"
                        else 0
                    )
                    * simulation_input_dict["controller"]["controller_dt"]
                )
        ...
        assert stoptime > 0, (
            "stoptime must be greater than 0, check stoptime, controller_dt, n_horizon, and prediction_timedelta."
        )

        simulation_mag = (simulation_u**2 + simulation_v**2) ** 0.5
        simulation_dir = 180.0 + np.rad2deg(np.arctan2(simulation_u, simulation_v))
        simulation_dir[simulation_dir < 0] = 360.0 + simulation_dir[simulation_dir < 0]
        simulation_dir[simulation_dir > 360] = np.mod(simulation_dir[simulation_dir > 360], 360.0)
        del simulation_u, simulation_v
    ...
    if wind_forecast_class:
        wind_forecast = wind_forecast_class(
            true_wind_field=kwargs["wind_field_ts"]
            ...
            **{k: v for k, v in simulation_input_dict["wind_forecast"].items() if "timedelta" in k},
            kwargs={
                k: v
                for k, v in simulation_input_dict["wind_forecast"].items()
                if "timedelta" not in k
            },
        )
    ...
    ctrl = controller_class(
        fi, wind_forecast=wind_forecast, simulation_input_dict=simulation_input_dict, **kwargs
    )

    n_future_steps = int(ctrl.controller_dt // simulation_input_dict["simulation_dt"]) - 1

    if load_from_checkpoint:
        fi.time = fi.init_time + timedelta(
            seconds=(t - simulation_input_dict["controller"]["controller_dt"])
        )
        ctrl.controls_dict = {
            "yaw_angles": np.array(
                [ctrl.yaw_IC] * ctrl.n_turbines if isinstance(ctrl.yaw_IC, float) else ctrl.yaw_IC
            )
        }
        ...
        # fi.env.core.farm.yaw_angles =
            ...
            disturbances={
                "wind_speeds": simulation_mag[previous_k : previous_k + n_future_steps + 1],
                "wind_directions": simulation_dir[previous_k : previous_k + n_future_steps + 1],
                "turbulence_intensities": [fi.env.core.flow_field.turbulence_intensities[0]]
                * (n_future_steps + 1),
            },
        ...
        sim_u = simulation_mag[previous_k] * np.sin(np.deg2rad(180.0 + simulation_dir[previous_k]))
        sim_v = simulation_mag[previous_k] * np.cos(np.deg2rad(180.0 + simulation_dir[previous_k]))
        ctrl.current_freestream_measurements = [sim_u, sim_v]
        ...
        ctrl.historic_measurements = (
            kwargs["wind_field_ts"]
            .filter(pl.col("time") < (ctrl.init_time + timedelta(seconds=t)))
            .select(
                ["time"]
                + ctrl.ws_horz_cols
                + ctrl.ws_vert_cols
            ...
            .with_columns(
                pl.col("time").cast(pl.Datetime(time_unit="ns")), cs.numeric().cast(pl.Float32)
            )
        )
    ...
    controller_signals_ts = (
        [[np.nan] * (ctrl.n_turbines if controller_class.__name__ == "GreedyController" else 3)]
        if k == 0
        else []
    )
    ...
    logging.info(
        f"Running for stoptime = {stoptime} for instance of {controller_class.__name__} - {kwargs['case_name']} with wind seed {kwargs['wind_case_idx']}"
    )
    while t < stoptime:
        ...
        if k == 0:
            ctrl_dict = {
                "yaw_angles": [ctrl.yaw_IC] * ctrl.n_turbines
                if isinstance(ctrl.yaw_IC, float)
                else ctrl.yaw_IC
            }
        elif k > 0:
            ctrl_dict = None
        ...
        sim_mag, sim_dir = (
            simulation_mag[k : k + n_future_steps + 1],
            simulation_dir[k : k + n_future_steps + 1],
        )
        ...
        sim_u = simulation_mag[k] * np.sin(np.deg2rad(180.0 + simulation_dir[k]))
        sim_v = simulation_mag[k] * np.cos(np.deg2rad(180.0 + simulation_dir[k]))
        ...
        ctrl_dt = (
            len(sim_dir) * simulation_input_dict["simulation_dt"]
        )  # can be less than 5s at end of time series
        for tt in np.arange(t, t + ctrl_dt, simulation_input_dict["simulation_dt"]):
            ...
            if (
                wind_forecast_class
                and kwargs["include_prediction"]
                and (
                    simulation_input_dict["wind_forecast"]["prediction_timedelta"].total_seconds()
                    > 0
                )
                and (ctrl.controls_dict["predicted_wind_speeds"] is not None)
            ):
            ...
            turbine_offline_status_ts += [
                np.isclose(ctrl.measurements_dict["turbine_powers"], 0, atol=1e-3)
            ]
        ...
        # if ctrl.verbose:
        logging.info(
            f"Time = {t}/{stoptime} of {controller_class.__name__} and {wind_forecast_class.__name__} - {kwargs['case_name']} with wind seed {kwargs['wind_case_idx']}"
        )
        if ctrl.verbose and False:
            logging.info(
                f"Measured Freestream Wind Direction = {simulation_dir[k]}",
        ...
        if (
            final
            or (
                (len(turbine_powers_ts) > 100)
                and ((ram_used := virtual_memory().percent) > kwargs["ram_limit"])
            )
            or (len(turbine_powers_ts) >= int(3600 / simulation_input_dict["simulation_dt"]))
        ):
...
def write_df(
    ...
    include_controller_signals=True,
):
    ...
    running_opt_cost_terms_ts[:, 0] = np.sum(
        np.stack(
            [-0.5 * (norm_turbine_powers[:, i]) ** 2 * Q for i in range(ctrl.n_turbines)], axis=1
        ),
        axis=1,
    )
    ...
    if start_step >= 0:
        slc = slice(start_step - 1, start_step - 1 + yaw_angles_ts.shape[0])
        fs_wind_u = simulation_mag[slc] * np.sin(np.deg2rad(180.0 + simulation_dir[slc]))
        fs_wind_v = simulation_mag[slc] * np.cos(np.deg2rad(180.0 + simulation_dir[slc]))
        ...
        filtered_fs_wind_u = first_ord_filter(
            fs_wind_u,
            alpha=np.exp(
                -(1 / simulation_input_dict["controller"]["wind_mag_lpf_time_const"])
                * simulation_input_dict["simulation_dt"]
            ),
        )
        filtered_fs_wind_v = first_ord_filter(
            fs_wind_v,
            alpha=np.exp(
                -(1 / simulation_input_dict["controller"]["wind_mag_lpf_time_const"])
                * simulation_input_dict["simulation_dt"]
        ...
        slc = slice(0, yaw_angles_ts.shape[0] - 1)
        fs_wind_u = np.insert(
            simulation_mag[slc] * np.sin(np.deg2rad(180.0 + simulation_dir[slc])), 0, np.nan
        )
        fs_wind_v = np.insert(
            simulation_mag[slc] * np.cos(np.deg2rad(180.0 + simulation_dir[slc])), 0, np.nan
        )
        ...
        filtered_fs_wind_u = np.insert(
            first_ord_filter(
                fs_wind_u[~np.isnan(fs_wind_u)],
                alpha=np.exp(
                    -(1 / simulation_input_dict["controller"]["wind_mag_lpf_time_const"])
                    * simulation_input_dict["simulation_dt"]
                ),
        ...
        filtered_fs_wind_v = np.insert(
            first_ord_filter(
                fs_wind_v[~np.isnan(fs_wind_v)],
                alpha=np.exp(
                    -(1 / simulation_input_dict["controller"]["wind_mag_lpf_time_const"])
                    * simulation_input_dict["simulation_dt"]
                ),
    ...
    del filtered_fs_wind_u, filtered_fs_wind_v

    start_step = max(0, start_step)

    results_data = {
        "Time": start_time
        + (np.arange(0, yaw_angles_ts.shape[0]) * simulation_input_dict["simulation_dt"]),
        ...
        **{
            f"TurbineYawAngle_{idx2tid_mapping[sorted_tids[i]]}": yaw_angles_ts[:, i]
            for i in range(ctrl.n_turbines)
        },
        **{
            f"TurbinePower_{idx2tid_mapping[sorted_tids[i]]}": turbine_powers_ts[:, i]
            for i in range(ctrl.n_turbines)
        },
        **{
            f"TurbineWindMag_{idx2tid_mapping[sorted_tids[i]]}": turbine_wind_mag_ts[:, i]
            for i in range(ctrl.n_turbines)
        },
        **{
            f"TurbineWindDir_{idx2tid_mapping[sorted_tids[i]]}": turbine_wind_dir_ts[:, i]
            for i in range(ctrl.n_turbines)
        },
        **{
            f"TurbineOfflineStatus_{idx2tid_mapping[sorted_tids[i]]}": turbine_offline_status_ts[
                :, i
            ]
            for i in range(ctrl.n_turbines)
        },
        ...
        **{
            f"RunningOptimizationCostTerm_{i}": running_opt_cost_terms_ts[:, i]
            for i in range(running_opt_cost_terms_ts.shape[1])
        },
    }
    ...
    if wf_source == "scada" and include_prediction:
        results_data.update(
            {
                **{
                    f"TrueTurbineWindSpeedHorz_{idx2tid_mapping[sorted_tids[i]]}": wind_field_ts.select(
                        f"ws_horz_{idx2tid_mapping[sorted_tids[i]]}"
                    )
                    .slice(start_step, yaw_angles_ts.shape[0])
                    .to_numpy()[:, 0]
                    for i in range(ctrl.n_turbines)
                },
                **{
                    f"TrueTurbineWindSpeedVert_{idx2tid_mapping[sorted_tids[i]]}": wind_field_ts.select(
                        f"ws_vert_{idx2tid_mapping[sorted_tids[i]]}"
                    )
                    .slice(start_step, yaw_angles_ts.shape[0])
                    .to_numpy()[:, 0]
                    for i in range(ctrl.n_turbines)
    ...
    if include_controller_signals:
        ...
        results_data.update(
            **{
                f"TurbineControllerSignal_{idx2tid_mapping[sorted_tids[i]]}": controller_signals_ts[
                    :, i
                ]
                for i in range(ctrl.n_turbines)
            }
    ...
    if (
        wind_forecast_class
        and include_prediction
        and simulation_input_dict["wind_forecast"]["prediction_timedelta"].total_seconds() > 0
    ):
        predicted_wind_speeds_ts = pl.concat(predicted_wind_speeds_ts, how="vertical").with_columns(
            time=((pl.col("time") - ctrl.init_time).dt.total_seconds().cast(pl.Float32))
        )

        cols = (
            ["time"]
            + ctrl.target_mean_ws_horz_cols
            + ctrl.target_mean_ws_vert_cols
            + (
                (ctrl.target_sd_ws_horz_cols + ctrl.target_sd_ws_vert_cols)
                if ctrl.uncertain
                else []
            )
        )
        predicted_wind_speeds_ts = (
            predicted_wind_speeds_ts.select(cols)
            .rename(
                {
        ...
        results_data = results_data.merge(
            predicted_wind_speeds_ts.to_pandas(), on=["Time"], how="outer"
        )
    ...
    if final and os.path.exists(temp_save_path):
        ...
        # os.rename(save_path.replace(".csv", "_x.csv"), save_path)
        # os.remove(os.path.join(save_path, save_path.replace(".csv", "_temp.csv")))

        # results_data.groupby("Time").last().to_csv(save_path, mode="a", header=False, index=False)

        # if (nunq := results_data["Time"].nunique()) < len(results_data):
