...
case_studies = {
    "baseline_controllers_forecasters_test_flasc": {
        ...
        "controller_class": {
            "group": 1,
            "vals": ["GreedyController", "LookupBasedWakeSteeringController"],
        },
        ...
        "wind_forecast_class": {
            "group": 3,
            "vals": ["KalmanFilterForecast"],
        },  # , ", "KalmanFilterForecast", "SpatialFilterForecast"]},
    ...
    "baseline_controllers_forecasters_test_awaken": {
        ...
        "prediction_timedelta": {"group": 1, "vals": [60]},
    ...
    "control_signal_test_awaken": {
        ...
        #     os.path.join(os.path.dirname(wind_forecasting_file), "../config/training/training_inputs_kestrel_awaken_pred_smoothed.yaml")]},
    ...
    "baseline_controllers_informer_forecaster_test_awaken": {
        ...
        # os.path.join(os.path.dirname(wind_forecasting_file), "../config/training/training_inputs_aoifemac_awaken_pred_smoothed.yaml")]},
        "controller_class": {"group": 1, "vals": ["LookupBasedWakeSteeringController"]},
        "prediction_timedelta": {"group": 1, "vals": [60]},
        "interpolation_method": {"group": 2, "vals": ["linear", "nearest"]},
        "filter_floris_wind": {"group": 3, "vals": [True, False]},
        "use_upstream_wind": {"group": 4, "vals": [True, False]},
        "use_lut_filtered_wind_mag": {"group": 5, "vals": [False, True]},
    },
    "baseline_controllers_svr_forecaster_test_awaken": {
        "controller_dt": {"group": 0, "vals": [5]},
        "simulation_dt": {"group": 0, "vals": [1]},
        "floris_input_file": {"group": 0, "vals": ["../../examples/inputs/gch_KP_v4.yaml"]},
        "yaw_limits": {"group": 0, "vals": ["-15,15"]},
        "uncertain": {"group": 0, "vals": [False]},
        "wind_forecast_class": {"group": 0, "vals": ["SVRForecast"]},
        "target_turbine_indices": {"group": 1, "vals": ["74,73"]},
        "model_config_path": {
            "group": 1,
            "vals": [
    ...
    "baseline_controllers_perfect_forecaster_flasc": {
        ...
        "prediction_timedelta": {
            "group": 2,
            "vals": [0, 60, 120, 180, 240, 300, 360, 420, 480, 540, 600, 660, 720, 780],
        },
    },
    ...
    "baseline_controllers_baseline_det_forecasters_awaken": {
        ...
        "controller_class": {
            "group": 1,
            "vals": ["LookupBasedWakeSteeringController", "GreedyController"],
        },
    ...
    "baseline_controllers": {
        "controller_dt": {"group": 1, "vals": [5, 5]},
    ...
    "wind_preview_type": {
        ...
        "case_names": {
            ...
            "vals": [
                "Perfect",
                "Persistent",
                "Stochastic Interval Elliptical 3",
                "Stochastic Interval Elliptical 5",
                "Stochastic Interval Elliptical 11",
                "Stochastic Interval Rectangular 3",
                "Stochastic Interval Rectangular 5",
                "Stochastic Interval Rectangular 11",
                "Stochastic Sample 25",
                "Stochastic Sample 50",
        ...
        "decay_type": {
            "group": 1,
            "vals": [None] * 2 + ["exp"] * 3 + ["none"] * 3 + ["cosine"] * 3,
        },
        "max_std_dev": {"group": 1, "vals": [None] * 2 + [2] * 3 + [2] * 3 + [1] * 3},
        "wind_preview_type": {
            "group": 1,
            "vals": ["perfect", "persistent"]
            + ["stochastic_interval_elliptical"] * 3
            + ["stochastic_interval_rectangular"] * 3
            + ["stochastic_sample"] * 3,
        },
    },
    "warm_start": {
    ...
    "breakdown_robustness":  # case_families[5]
    {
        "controller_class": {
            "group": 1,
            "vals": ["MPC", "LookupBasedWakeSteeringController", "GreedyController"],
        },
    ...
    "scalability": {
        ...
        "floris_input_file": {
            ...
            "vals": [
                os.path.join(
                    os.path.dirname(whoc_file),
                    "../examples/mpc_wake_steering_florisstandin",
                    f"floris_gch_{i}.yaml",
                )
                for i in [3, 9, 25]
            ],
        },
        "lut_path": {
            ...
            "vals": [
                os.path.join(
                    os.path.dirname(whoc_file),
                    f"../examples/mpc_wake_steering_florisstandin/lookup_tables/lut_{nturb}.csv",
                )
                for nturb in [3, 9, 25]
            ],
        },
    ...
    "cost_func_tuning": {
        ...
        "case_names": {
            "group": 1,
            "vals": [
                f"alpha_{np.round(f, 3)}"
                for f in list(
                    np.concatenate(
                        [
                            np.linspace(0, 0.8, int(N_COST_FUNC_TUNINGS // 2)),
                            0.801
                            + (
                                1
                                - np.logspace(
                                    -3, 0, N_COST_FUNC_TUNINGS - int(N_COST_FUNC_TUNINGS // 2)
                                )
                            )
                            * 0.199,
                        ]
    ...
    "yaw_offset_study": {
        "controller_class": {
            "group": 1,
            "vals": ["MPC", "MPC", "MPC", "LookupBasedWakeSteeringController", "MPC", "MPC"],
        },
        "case_names": {
            "group": 1,
            "vals": [
                f"StochasticIntervalRectangular_1_3turb",
                f"StochasticIntervalRectangular_11_3turb",
                f"StochasticIntervalElliptical_11_3turb",
                f"LUT_3turb",
                f"StochasticSample_25_3turb",
                f"StochasticSample_100_3turb",
            ],
        },
        "wind_preview_type": {
            "group": 1,
            "vals": ["stochastic_interval_rectangular"] * 2
    ...
    "baseline_plus_controllers": {
        "controller_dt": {"group": 1, "vals": [5, 5, 60.0, 60.0, 60.0, 60.0]},
        "case_names": {
            "group": 1,
            "vals": [
                "LUT",
                ...
                "MPC_with_Filter",
                "MPC_without_Filter",
                "MPC_without_state_cons",
                "MPC_without_dyn_state_cons",
            ],
        },
        "controller_class": {
            "group": 1,
    ...
    "gradient_type": {
        ...
        "wind_preview_type": {
            "group": 1,
            "vals": ["stochastic_interval_rectangular"] * 4
            + ["stochastic_interval_elliptical"] * 4
            + ["stochastic_sample"] * 6,
        },
        "n_wind_preview_samples": {"group": 1, "vals": [10] * 4 + [33] * 4 + [100] * 6},
        "diff_type": {
            "group": 1,
            "vals": ["direct_cd", "direct_fd", "chain_cd", "chain_fd"] * 2
            + ["direct_cd", "direct_fd", "direct_zscg", "chain_cd", "chain_fd", "chain_zscg"],
        },
    ...
    "n_wind_preview_samples": {
        ...
        "case_names": {
            ...
            "vals": [
                "Stochastic Interval Elliptical 11",
                "Stochastic Interval Elliptical 21",
                "Stochastic Interval Elliptical 33",
                "Stochastic Interval Rectangular 5",
                "Stochastic Interval Rectangular 7",
                "Stochastic Interval Rectangular 11",
                "Stochastic Sample 25",
                "Stochastic Sample 50",
                "Stochastic Sample 100",
        ...
        "decay_type": {
            "group": 1,
            "vals": ["exp"] * 4 + ["cosine"] * 4 + ["exp"] * 4 + ["none", "none"],
        },
        "n_wind_preview_samples": {
            "group": 1,
            "vals": [11, 21, 33] + [5, 7, 11] + [25, 50, 100] + [1, 1],
        },
        "diff_type": {
            "group": 1,
            "vals": ["direct_cd"] * 3
        ...
        "wind_preview_type": {
            "group": 1,
            "vals": ["stochastic_interval_elliptical"] * 3
            + ["stochastic_interval_rectangular"] * 3
            + ["stochastic_sample"] * 3
            + ["perfect", "persistent"],
        },
    ...
    "cost_func_tuning_small": {
        ...
        "case_names": {
            "group": 1,
            "vals": [f"alpha_{np.round(f, 3)}" for f in [0.0, 0.001, 0.5, 0.999, 1]],
        },
...
def CaseGen_General(case_inputs, namebase=""):
    ...
    # index of each group
    matrix_group_idx = [
        np.where([group_i == group_j for group_j in change_group])[0].tolist()
...
def initialize_simulations(
    ...
    simulation_dt = set(
        np.concatenate([case_studies[k]["simulation_dt"]["vals"] for k in case_study_keys])
    )
    ...
    if (
        "slsqp_solver_sweep" not in case_studies
        or "controller_dt" not in case_studies["slsqp_solver_sweep"]
    ):
    ...
    if (whoc_config["controller"]["target_turbine_indices"] is not None) and (
        (num_target_turbines := len(whoc_config["controller"]["target_turbine_indices"]))
        < whoc_config["controller"]["num_turbines"]
    ):
        ...
        floris_input_file = os.path.splitext(
            os.path.basename(whoc_config["controller"]["floris_input_file"])
        )[0]
        yaw_limits = (whoc_config["controller"]["yaw_limits"])[1]
        target_turbine_indices = tuple(
            int(i) for i in whoc_config["controller"]["target_turbine_indices"]
        )
        uncertain_flag = whoc_config["controller"]["uncertain"]
        whoc_config["controller"]["lut_path"] = os.path.join(
        ...
        whoc_config["controller"]["target_turbine_indices"] = tuple(
            whoc_config["controller"]["target_turbine_indices"]
        )
    ...
    if wf_source == "floris":
        ...
        from whoc.wind_field.WindField import (
            generate_multi_wind_ts,
            WindField,
            write_abl_velocity_timetable,
            first_ord_filter,
        )

        with open(
            os.path.join(os.path.dirname(whoc_file), "wind_field", "wind_field_config.yaml"), "r"
        ) as fp:
        ...
        wind_field_config["n_preview_steps"] = int(
            wind_field_config["simulation_max_time"] / whoc_config["simulation_dt"]
        ...
        wind_field_config["distribution_params_path"] = os.path.join(
            save_dir, "wind_field_data", "wind_preview_distribution_params.pkl"
        )
        ...
        if len(wind_field_filenames) < n_seeds or regenerate_wind_field:
            ...
            wind_field_config["regenerate_distribution_params"] = (
                True  # set to True to regenerate from constructed mean and covaraicne
            )
            full_wf = WindField(**wind_field_config)
            os.makedirs(wind_field_dir, exist_ok=True)
            wind_field_data = generate_multi_wind_ts(
                full_wf, wind_field_dir, init_seeds=[seed + i for i in range(n_seeds)]
            )
            write_abl_velocity_timetable(
                [wfd.df for wfd in wind_field_data], wind_field_dir
            )  # then use these timetables in amr precursor
            # write_abl_velocity_timetable(wind_field_data, wind_field_dir) # then use these timetables in amr precursor
            wind_dir_lpf_alpha = np.exp(
                -(1 / whoc_config["controller"]["wind_dir_lpf_time_const"])
                * whoc_config["simulation_dt"]
            )
            # wind_mag_lpf_alpha = np.exp(-(1 / whoc_config["controller"]["wind_mag_lpf_time_const"]) * whoc_config["simulation_dt"])
            plot_wind_field_ts(
                wind_field_data[0].df,
                wind_field_dir,
                filter_func=partial(first_ord_filter, alpha=wind_dir_lpf_alpha),
            )
            plot_ts(pd.concat([wfd.df for wfd in wind_field_data]), wind_field_dir)
            wind_field_filenames = [
                os.path.join(wind_field_dir, f"case_{i}.csv") for i in range(n_seeds)
            ]
        ...
        if os.path.exists(wind_field_dir):
            for f, fn in enumerate(wind_field_filenames):
                wind_field_ts.append(
                    pl.read_csv(fn, try_parse_dates=True)
        ...
        assert np.all(
            [
                np.isclose(
                    wind_field_ts[case_idx]
                    .select(pl.col("time").diff().slice(1, 1).dt.total_seconds())
                    .item(),
                    whoc_config["simulation_dt"],
                )
                for case_idx in range(n_seeds)
            ]
        ), "sampling time of wind field should be equal to simulation sampling time"

        wind_field_ts = [
            wind_field_ts[case_idx].select(["time", "FreestreamWindMag", "FreestreamWindDir"])
            for case_idx in range(n_seeds)
        ]

        if stoptime == "auto":
            durations = [
                (df.select((pl.col("time").last() - pl.col("time").first()).dt.total_seconds()))
            ...
            whoc_config["hercules_comms"]["helics"]["config"]["stoptime"] = stoptime = [
                d.total_seconds() if hasattr(d, "total_seconds") else d for d in durations
            ]
        else:
        ...
        data_module = DataModule(
            normalized_data_path=base_model_config["dataset"]["data_path"],
            ...
            train_split=(
                1.0
                - base_model_config["dataset"]["val_split"]
                - base_model_config["dataset"]["test_split"]
            ),
            val_split=base_model_config["dataset"]["val_split"],
            test_split=base_model_config["dataset"]["test_split"],
        ...
        data_module.generate_splits(
            splits=["test"], save=True, reload=reload
        )  # TODO should reload if context/prediction length has changed
        ...
        if n_seeds != "auto":
            ...
            logging.info(
                f"Durations in wind_field_ts = {[np.round(df.select((pl.col('time').last() - pl.col('time').first())).item().total_seconds() / 3600, 2) for df in wind_field_ts]} hours"
            )

        n_seeds = len(wind_field_ts)

        wind_dt = (
            wind_field_ts[0].select(pl.col("time").diff().slice(1, 1).dt.total_seconds()).item()
        )
        logging.info(
            f"Loaded and normalized SCADA wind field from {base_model_config['dataset']['data_path']} with dt = {wind_dt} seconds."
        )
        ...
        if simulation_dt != wind_dt:
            ...
            wind_field_ts = [
                wf.set_index("time")
                .resample(f"{simulation_dt}s")
                .mean()
                .reset_index(names=["time"])
                for wf in wind_field_ts
            ]

            if wind_dt < simulation_dt:
                wind_field_ts = [
                    wf.with_columns(time=pl.col("time").dt.round(simulation_dt))
                ...
                wind_field_ts = [
                    wf.upsample(time_column="time", every=f"{simulation_dt}s").interpolate()
                    for wf in wind_field_ts
                ]
        ...
        if False:
            ...
            check_seed_times = []
            with open(
                "/Users/ahenry/Documents/toolboxes/wind_forecasting/wind_forecasting/run_scripts/perfect_testing.txt"
            ) as fp:
                ...
                wind_field_ts = [
                    wind_field_ts[seed].loc[
                        (wind_field_ts[seed]["time"] - wind_field_ts[seed]["time"].iloc[0])
                        .dt.total_seconds()
                        .between(0, time + 120),
                        :,
                    ]
        ...
        if stoptime == "auto":
            durations = [
                df.select(pl.col("time").last() - pl.col("time").first()).item()
                for df in wind_field_ts
            ]
            if any(
                int(d / pd.Timedelta(data_module.freq))
                < data_module.context_length + data_module.prediction_length
                for d in durations
            ):
                logging.warning(
                    f"One or more continuity groups are too short, delete train ready path {data_module.train_ready_path} to reload."
                )
            ...
            wind_field_ts = [
                wf.filter((pl.col("time") - pl.col("time").first()).dt.total_seconds() <= stoptime)
                for wf in wind_field_ts
            ]
            stoptime = [stoptime] * len(wind_field_ts)
    ...
    for case_family in case_families:
        ...
        case_studies[case_family]["wind_case_idx"] = {
            "group": 0,
            "vals": [i for i in range(n_seeds)],
        }
    ...
    for case_study_key in case_study_keys:
        ...
        for k in group_val_lengths:
            assert len(set(group_val_lengths[k])) == 1, (
                f"Value lists for group {k - 1} of {case_study_key} have different number of elements."
            )
        ...
        for c, case in enumerate(case_list):
            ...
            for property_name, property_value in case.items():
                ...
                elif (property_name in input_dicts[start_case_idx + c]["wind_forecast"]) or (
                    input_dicts[start_case_idx + c]["controller"]["wind_forecast_class"]
                    and property_name
                    in input_dicts[start_case_idx + c]["wind_forecast"].get(
                        input_dicts[start_case_idx + c]["controller"]["wind_forecast_class"], {}
                    )
                ):
                ...
                if property_group:
                    if property_name == "yaw_limits":
                        input_dicts[start_case_idx + c][property_group][property_name] = tuple(
                            int(v) for v in str(property_value).split(",")
                        )
                    elif property_name == "target_turbine_indices":
                        if property_value != "all":
                            # need to preserve order, taking first as upstream
                            target_turbine_indices = np.array(
                                [int(v) for v in property_value.split(",") if len(v)]
                            )
                            ...
                            input_dicts[start_case_idx + c]["controller"][property_name] = tuple(
                        ...
                        if (
                            case.get(
                                "controller_class", whoc_config["controller"]["controller_class"]
                            )
                        ) == "GreedyController":
                            ...
                            input_dicts[start_case_idx + c]["controller"]["uncertain"] = (
                                property_value
                            )
                    elif isinstance(property_value, np.str_):
                        input_dicts[start_case_idx + c][property_group][property_name] = str(
                            property_value
                        )
                    else:
                        input_dicts[start_case_idx + c][property_group][property_name] = (
                            property_value
                        )
                else:
            ...
            assert all(
                input_dicts[start_case_idx + c]["controller"]["controller_dt"] <= t
                for t in stoptime
            )

            if (
                input_dicts[start_case_idx + c]["controller"]["wind_forecast_class"]
                or "wind_forecast_class" in case
            ):
                if input_dicts[start_case_idx + c]["wind_forecast"]["model_config_path"] is None:
                    mdl_cnf = base_model_config

                else:
                    model_config_path = input_dicts[start_case_idx + c]["wind_forecast"][
                        "model_config_path"
                    ]
                ...
                if (
                    input_dicts[start_case_idx + c]["controller"]["wind_forecast_class"]
                    in ["MLForecast", "SVRForecast"]
                ) and (
                    input_dicts[start_case_idx + c]["wind_forecast"]["prediction_timedelta"]
                    > mdl_cnf["dataset"]["prediction_length"]
                ):
                    logging.warning(
                        f"Provided prediction_timedelta {input_dicts[start_case_idx + c]['wind_forecast']['prediction_timedelta']} should be less or equal to the trained model config prediction length {mdl_cnf['dataset']['prediction_length']}. Make sure you are providing the right model config file. Resetting the prediction_timedelta variable."
                    )
                    input_dicts[start_case_idx + c]["wind_forecast"]["prediction_timedelta"] = (
                        mdl_cnf["dataset"]["prediction_length"]
                    )
                ...
                wind_forecast_kwargs = {
                    ...
                    "context_timedelta": pd.Timedelta(
                        seconds=mdl_cnf["dataset"]["context_length"]
                    ),  # pd.Timedelta(seconds=input_dicts[start_case_idx + c]["wind_forecast"]["context_timedelta"]),
                    "prediction_timedelta": pd.Timedelta(
                        seconds=input_dicts[start_case_idx + c]["wind_forecast"][
                            "prediction_timedelta"
                        ]
                    ),
                    "controller_timedelta": pd.Timedelta(
                        seconds=input_dicts[start_case_idx + c]["controller"]["controller_dt"]
                    ),
                ...
                input_dicts[start_case_idx + c]["wind_forecast"].update(
                    {
                        k: v
                        for k, v in input_dicts[start_case_idx + c]["wind_forecast"]
                        .get(
                            input_dicts[start_case_idx + c]["controller"]["wind_forecast_class"], {}
                        )
                        .items()
                ...
                model_specific_keys = [
                    key
                    for key in input_dicts[start_case_idx + c]["wind_forecast"].keys()
            ...
            # need to change num_turbines, floris_input_file, lut_path
            if (
                target_turbine_indices := input_dicts[start_case_idx + c]["controller"][
                    "target_turbine_indices"
                ]
            ) != "all":
                ...
                lut_path = os.path.abspath(
                    input_dicts[start_case_idx + c]["controller"]["lut_path"]
                )
                floris_input_file = os.path.splitext(
                    os.path.basename(
                        input_dicts[start_case_idx + c]["controller"]["floris_input_file"]
                    )
                )[0]
            ...
            assert (
                input_dicts[start_case_idx + c]["controller"]["controller_dt"]
                >= input_dicts[start_case_idx + c]["simulation_dt"]
            ), "controller_dt must be greater than or equal to simulation_dt"
            ...
            if (
                (
                    case.get("controller_class", whoc_config["controller"]["controller_class"])
                    in ["LookupBasedWakeSteeringController", "MPC"]
                )
                and regenerate_lut
                or not os.path.exists(input_dicts[start_case_idx + c]["controller"]["lut_path"])
            ):
                floris_input_file = input_dicts[start_case_idx + c]["controller"][
                    "floris_input_file"
                ]
                ...
                if (
                    ...
                    LookupBasedWakeSteeringController._optimize_lookup_table(
                        floris_config_path=floris_input_file,
                        uncertain=uncertain_flag,
                        yaw_limits=yaw_limits,
                        parallel=multiprocessor is not None,
                        sorted_target_tids=sorted(target_turbine_indices)
            ...
            if "case_names" not in case_lists[start_case_idx + c]:
                ...
                input_dicts[start_case_idx + c]["case_name"] = str(
                    c % int(len(case_list) / n_seeds)
                )
            else:
                input_dicts[start_case_idx + c]["case_name"] = case_lists[start_case_idx + c][
                    "case_names"
                ]
            ...
            input_filenames.append(
                (
                    case_study_key,
                    input_dicts[start_case_idx + c]["case_name"],
                    input_dicts[start_case_idx + c]["wind_case_idx"],
                    fn,
                )
            )
            ...
            if existing_input_df is not None:
                ...
                if len(existing_input_df.loc[cond, :].index):
                    ...
                    if str(existing_case_no) != input_dicts[start_case_idx + c]["case_name"]:
                        old2new_case_names[existing_case_no] = input_dicts[start_case_idx + c][
                            "case_name"
                        ]
                        existing_fn = os.path.join(
                            save_dir,
                            case_study_key,
                            f"time_series_results_case_{existing_case_no}_seed_{input_dicts[start_case_idx + c]['wind_case_idx']}.csv",
                        )
                        new_fn = os.path.join(
                            save_dir,
                            case_study_key,
                            f"time_series_results_case_{input_dicts[start_case_idx + c]['case_name']}_seed_{input_dicts[start_case_idx + c]['wind_case_idx']}_existing.csv",
                        )
                        ...
                        if os.path.exists(existing_fn):
                            logging.info(
                                f"Found existing temporarly time_series_results file {existing_fn}."
                            )
                            logging.info(f"Renaming {existing_fn} to {new_fn}.")
                            os.rename(existing_fn, new_fn)
            ...
            if case_lists[start_case_idx + c]["wind_case_idx"] == 0:
                ...
                sim_data = {
                    **{"simulation_dt": input_dicts[start_case_idx + c]["simulation_dt"]},
                    **input_dicts[start_case_idx + c]["controller"],
                    **input_dicts[start_case_idx + c]["wind_forecast"],
                }
                input_df.append(
                    pd.DataFrame(data={k: [v] for k, v in sim_data.items() if k != "wind_case_idx"})
                )
        ...
        for fn in glob(
            os.path.join(
                save_dir, case_study_key, f"time_series_results_case_*_seed_*_existing.csv"
            )
        ):
    ...
    if False:
        for case_study_key in case_study_keys:
            ...
            for inp_file in glob(os.path.join(results_dir, "input_config_case_*.pkl")):
                ...
                try:
                    ctrl_cls = re.search(
                        "(?<=controller_class_)(\\w+)(?=_controller_dt)", fn
                    ).group()
                    ctrl_dt = int(
                        re.search("(?<=controller_dt_)(\\d+)(?=_prediction_timedelta)", fn).group()
                    )
                    prediction_timedelta = int(
                        re.search(
                            "(?<=prediction_timedelta_)(\\d+)(?=_target_turbine_indices)", fn
                        ).group()
                    )
                    tgt_turb_ind = re.search(
                        "(?<=target_turbine_indices_)(.*)(?=_uncertain)", fn
                    ).group()
                    unc_flag = (
                        True
                        if re.search("(?<=uncertain_)(.*)(?=_wind_forecast_class)", fn).group()
                        == "True"
                        else False
                    )
                    wind_fct_cls = re.search("(?<=wind_forecast_class_)(\\w+)(?=.pkl)", fn).group()
                    new_case_name = inp_info.loc[
                        (inp_info["controller_class"] == ctrl_cls)
                        & (inp_info["controller_dt"] == ctrl_dt)
                        & (inp_info["prediction_timedelta"] == prediction_timedelta)
                        & (inp_info["target_turbine_indices"] == tgt_turb_ind)
                        & (inp_info["uncertain"] == unc_flag)
                        & (inp_info["wind_forecast_class"] == wind_fct_cls),
                        :,
                    ].index[0]
                    shutil.move(
                        inp_file,
            ...
            for ts_file in glob(os.path.join(results_dir, "*.csv")):
                ...
                try:
                    ...
                    wind_fct_cls = re.search(
                        "(?<=wind_forecast_class_)(\\w+)(?=_seed_\\d+.csv)", fn
                    ).group()
                    new_case_name = inp_info.loc[
                        (inp_info["controller_class"] == ctrl_cls)
                        & (inp_info["controller_dt"] == ctrl_dt)
                    ...
                    shutil.move(
                        ...
                        os.path.join(
                            results_dir,
                            re.sub(
                                "(?<=time_series_results_case_)(.*)(?=_seed_\\d+\\.csv)",
                                str(new_case_name),
    ...
    if run_simulations:
        ...
        ts_filenames = [
            tuple([csk, f"time_series_results_case_{case_name}_seed_{wind_case_idx}.csv"])
            for csk, case_name, wind_case_idx, fn in input_filenames
        ]
        for case_study_key in case_study_keys:
            allowed_input_files = set(
                [fn for csk, _, _, fn in input_filenames if csk == case_study_key]
            )
    ...
    for f, ((case_study_key, case_name, wind_case_idx, fn), inp) in enumerate(
        zip(input_filenames, input_dicts)
    ):
        ...
        if inp_path not in written_input_files:
            logging.info(
                f"Writing input_config file {len(written_input_files) + 1} of {total_cases}"
            )
...
case_families = [
    ...
    "warm_start",  # 2, 3
    "horizon_length",
    "cost_func_tuning",  # 4, 5
    "yaw_offset_study",
    "scalability",  # 6, 7
    "breakdown_robustness",  # 8
    "gradient_type",
    "n_wind_preview_samples",  # 9, 10
    "generate_sample_figures",
    "baseline_controllers_3",  # 11, 12
    "cost_func_tuning_small",
    "sr_solve",  # 13, 14
    "baseline_controllers_informer_forecasters_awaken",
    "baseline_controllers_autoformer_forecasters_awaken",  # 15, 16
    "baseline_controllers_spacetimeformer_forecasters_awaken",
    "baseline_controllers_tactis_forecasters_awaken",  # 17, 18
    "baseline_controllers_baseline_det_forecasters_awaken",
    "baseline_controllers_baseline_prob_forecasters_awaken",  # 19, 20
    "baseline_controllers_perfect_forecaster_flasc",
    "baseline_controllers_perfect_forecaster_awaken",  # 21, 22
    "baseline_controllers_baseline_perfect0_forecasters_awaken",  # 23
    "baseline_controllers_forecasters_test_flasc",
    "baseline_controllers_forecasters_test_awaken",  # 24, 25
    "baseline_controllers_informer_forecaster_test_awaken",
