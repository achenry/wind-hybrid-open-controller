...
#SBATCH --job-name=flasc_valid_600s_tactis_fixed
#SBATCH --output=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/flasc_valid_600s_fixed_%j.out
#SBATCH --error=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/flasc_valid_600s_fixed_%j.err
...
export CHECKPOINT_ARG="/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/tune_tactis_flasc_3_local_tactis/20250722_210705_0_0/epoch=59-step=583680-val_loss=-38.73.ckpt" # Specific 600s checkpoint
...
mkdir -p ${LOG_DIR}/inference_results/flasc_validation_600s_fixed_${SLURM_JOB_ID}
...
# --- Print Job Info ---
echo "--- SLURM JOB INFO (600s Validation Fixed) ---"
...
echo "Starting WindForecast validation for 600s horizon (fixed)..."
date +"%Y-%m-%d %H:%M:%S"
...
python run_forecaster_validation.py \
    ...
    --save_dir "${LOG_DIR}/inference_results/flasc_validation_600s_fixed_${SLURM_JOB_ID}" \
...
if [ ${EXIT_CODE} -eq 0 ]; then
  echo "600s validation completed successfully."
else
  echo "ERROR: 600s validation failed with exit code ${EXIT_CODE}." >&2
fi
echo "---------------------------------------"
