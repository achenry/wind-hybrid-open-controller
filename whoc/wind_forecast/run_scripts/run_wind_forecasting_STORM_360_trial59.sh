...
#SBATCH --time=0-08:00              # Time limit (2 days for comprehensive validation)
#SBATCH --job-name=flasc_valid_360s_trial59
#SBATCH --output=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/flasc_valid_360s_trial59_%j.out
#SBATCH --error=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/flasc_valid_360s_trial59_%j.err
...
# --- Validation Settings for 360s Horizon with Trial 59 Checkpoint ---
...
# CRITICAL: Use the specific checkpoint from trial 59 training
export CHECKPOINT_ARG="/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/tune_tactis_flasc_9_greedy_tactis/20250817_124744_0_0/last.ckpt"

export MAX_STEPS_ARG=2160     # 360s prediction + 600s context + buffer
export PREDICTION_TYPE_ARG="sample"  # Use sample-based prediction for TACTiS

# --- Create Logging Directories ---
mkdir -p ${LOG_DIR}/slurm_logs
mkdir -p ${LOG_DIR}/inference_results/flasc_validation_360s_trial59_${SLURM_JOB_ID}
...
# --- Print Job Info ---
echo "--- SLURM JOB INFO (360s Validation with Trial 59 Checkpoint) ---"
...
echo "------------------------"
echo "CRITICAL: Using Trial 59 training checkpoint from job 11407669"
echo "This checkpoint includes 41 epochs of training with:"
echo "  - Stage 1 (epochs 0-19): Marginal flow training"
echo "  - Stage 2 (epochs 20-40): Copula training with improved learning rates"
...
if [ ! -f "${CHECKPOINT_ARG}" ]; then
  echo "ERROR: Checkpoint file not found: ${CHECKPOINT_ARG}" >&2
  exit 1
fi
echo "Verified checkpoint exists: ${CHECKPOINT_ARG}"
echo "Checkpoint size: $(ls -lh ${CHECKPOINT_ARG} | awk '{print $5}')"
echo "Checkpoint modified: $(ls -lh ${CHECKPOINT_ARG} | awk '{print $6, $7, $8}')"
echo "------------------------"

# --- Run Inference Script ---
echo "Starting WindForecast validation for 360s horizon with Trial 59 checkpoint..."
date +"%Y-%m-%d %H:%M:%S"
...
# Note: We do NOT use --use_tuned_params flag here because we want to use the specific
# checkpoint that already has the tuned parameters from trial 59 embedded in it
python run_forecaster_validation.py \
    ...
    --save_dir "${LOG_DIR}/inference_results/flasc_validation_360s_trial59_${SLURM_JOB_ID}" \
...
if [ ${EXIT_CODE} -eq 0 ]; then
  echo "360s validation with Trial 59 checkpoint completed successfully."
  echo "Results saved to: ${LOG_DIR}/inference_results/flasc_validation_360s_trial59_${SLURM_JOB_ID}"
else
...
# Usage: sbatch /user/taed7566/Forecasting/wind-hybrid-open-controller/whoc/wind_forecast/run_scripts/run_wind_forecasting_STORM_360_trial59.sh