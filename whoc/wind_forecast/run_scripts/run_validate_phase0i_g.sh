...
#SBATCH --job-name=tactis_validate_phase0i_g
#SBATCH --output=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/whoc_validate_phase0i_g_%j.out
#SBATCH --error=/dss/work/taed7566/Forecasting_Outputs/wind-forecasting/logs/slurm_logs/whoc_validate_phase0i_g_%j.err
#SBATCH --hint=nomultithread
#SBATCH --distribution=block:block
#SBATCH --gres-flags=enforce-binding

# =============================================================================
# Phase 0i-G validation — Quantile head + pinball loss (Path 2)
# =============================================================================
# Probabilistic validation of the Phase 0i-G checkpoint, using the same harness
# configuration as Phase 0i-B for apples-to-apples comparison via the
# wind_forecasting/postprocessing/compare_models.py tool.
#
# Submit (after production training finishes; UPDATE CHECKPOINT path below):
#   eval "$(grep '^export LOCAL_PG_PASSWORD=' ~/.zshrc)"
#   sbatch --partition=cfdg.p,all_gpu.p --export=ALL,LOCAL_PG_PASSWORD run_validate_phase0i_g.sh
# =============================================================================
...
# === Validation inputs ===
export MODEL="tactis"
# Phase 0i-G model config: quantile head + pinball loss
export MODEL_CONFIG="${WF_DIR}/config/training/training_inputs_storm_awaken_unsmoothed_pred60_tactis_phase0i_g.yaml"
# Phase 0i-G checkpoint: final production checkpoint (J1=16918365, 100 epochs full Stage1+Stage2)
# F^-1 width: 2.09 std-units at ep99 (vs 0.27 collapsed Phase 0i-B baseline; vs 2.19 G4 pilot)
export CHECKPOINT="${LOG_DIR}/train_phase0i_g_quantile_pinball_tactis/20260512_101511_0_0/manual_save_epoch99.ckpt"
