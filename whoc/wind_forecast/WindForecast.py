...
class WindForecast:
    ...
    def plot_forecast(forecast_wf, true_wf, continuity_groups=None, feature_types=None, feature_labels=None, prediction_type="point", per_turbine_target=False, turbine_ids="all", label="", fig_dir="./",
                      include_turbine_legend=False, include_forecast=False, boxcox_transform=False, boxcox_features=["ws_horz", "ws_vert"], multiple_forecasters=True,
        ...
        if continuity_groups is not None and "continuity_group" in true_wf.collect_schema().names():
            ...
            #true_times = true_wf.select("time").unique().to_series().to_list()
            #forecast_wf = forecast_wf.filter(pl.col("time").is_in(true_times))
        ...
        xlim_rng = x1 - x0
        new_xlim_upper = x1 + (new_time_range / time_rng) * xlim_rng
        new_xlim = axs[-1, -1].set_xlim(x0, new_xlim_upper) #(axs[-1, -1].get_xlim()[0], axs[-1, -1].get_xlim()[0] + (new_time_range/time_rng)*xlim_rng)
        n_ticks = 5
...
class TemporalFusionTransformerForecast(WindForecast):
    ...
    def find_optimal_learning_rate(self, trainer=None, max_epochs=10, batch_size=64):
        ...
        if self.model is None:
            self.model = TemporalFusionTransformer.from_dataset(self.data, learning_rate=0.01, hidden_size=8, attention_head_size=1, dropout=0.1, hidden_continuous_size=8, loss=QuantileLoss(), optimizer="ranger")
            print(f"Number of parameters in model: {self.model.size() / 1e3:.1f}k")

        if trainer is None:
             trainer = Trainer(max_epochs=max_epochs, accelerator="auto", logger=False, enable_checkpointing=False, enable_progress_bar=False)

        tuner = Tuner(trainer)
        lr_finder = tuner.lr_find(self.model, train_dataloaders=self.data.to_dataloader(train=True, batch_size=batch_size), min_lr=1e-6, max_lr=1e-1)
        suggested_lr = lr_finder.suggestion()

        fig = lr_finder.plot(suggest_lr=True)
        plt.show()
    ...
    def get_params(self, trial):
        ...
        optimizer = trial.suggest_categorical("optimizer", ["adam", "ranger"])
...
def generate_forecaster_agg_results(forecaster, forecast_df, test_data, data_module, prediction_type):
    ...
    # Steven_fdf fdf = forecast_df = pl.concat(forecast_df, how="vertical").select(["time"] + [cs.ends_with(tgt) for tgt in data_module.target_cols])
...
if __name__ == "__main__":
    ...
    if "tft" in args.model:
        for td in prediction_timedelta:
            forecaster = TemporalFusionTransformerForecast(measurements_timedelta=measurements_timedelta,
                                                            controller_timedelta=controller_timedelta,
                                                            prediction_timedelta=td, 
                                                            context_timedelta=context_timedelta,
                                                            fmodel=fmodel,
    ...
    if RUN_ONCE:
        ...
        arima_metrics = next(r["agg_metrics"] for r in results if r["forecaster_name"] == "ARIMAForecast")
        persistence_metrics = next(r["agg_metrics"] for r in results if r["forecaster_name"] == "PersistenceForecast")
