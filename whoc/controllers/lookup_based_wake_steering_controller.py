...
class LookupBasedWakeSteeringController(ControllerBase):
    def __init__(self, interface, wind_forecast, simulation_input_dict, verbose=False, **kwargs):
        ...
        self.controller_dt = simulation_input_dict["controller"][
            "controller_dt"
        ]  # Won't be needed here, but generally good to have
        self.n_turbines = (
            interface.n_turbines
        )  # simulation_input_dict["controller"]["num_turbines"]
        self.fi = interface.env

        self.prediction_timedelta_stored = max(
            pd.Timedelta(self.controller_dt, unit="s"), self.wind_forecast.prediction_timedelta
        )
        ...
        self.wind_dir_lpf_time_const = simulation_input_dict["controller"][
            "wind_dir_lpf_time_const"
        ]
        self.wind_mag_lpf_time_const = simulation_input_dict["controller"][
            "wind_mag_lpf_time_const"
        ]
        self.lpf_start_time = self.init_time + pd.Timedelta(
            seconds=simulation_input_dict["controller"]["lpf_start_time"]
        )
        self.wind_dir_lpf_alpha = np.exp(
            -(1 / simulation_input_dict["controller"]["wind_dir_lpf_time_const"])
            * simulation_input_dict["simulation_dt"]
        )
        self.wind_mag_lpf_alpha = np.exp(
        ...
        self.target_ws_horz_cols = [
            f"ws_horz_{self.idx2tid_mapping[t_idx]}" for t_idx in self.sorted_tids
        ]
        self.target_ws_vert_cols = [
            f"ws_vert_{self.idx2tid_mapping[t_idx]}" for t_idx in self.sorted_tids
        ]
        ...
        if self.wind_forecast and self.uncertain:
            ...
            self.target_mean_ws_horz_cols = [
                f"loc_ws_horz_{self.idx2tid_mapping[t_idx]}" for t_idx in self.sorted_tids
            ]
            self.target_mean_ws_vert_cols = [
                f"loc_ws_vert_{self.idx2tid_mapping[t_idx]}" for t_idx in self.sorted_tids
            ]
            self.target_sd_ws_horz_cols = [
                f"sd_ws_horz_{self.idx2tid_mapping[t_idx]}" for t_idx in self.sorted_tids
            ]
            self.target_sd_ws_vert_cols = [
                f"sd_ws_vert_{self.idx2tid_mapping[t_idx]}" for t_idx in self.sorted_tids
            ]
        else:
            ...
            self.sd_ws_horz_cols = self.sd_ws_vert_cols = self.target_sd_ws_horz_cols = (
                self.target_sd_ws_horz_cols
            ) = []
        ...
        if "df_yaw" in kwargs:
            ...
            self.wake_steering_interpolant = (
                LookupBasedWakeSteeringController._optimize_lookup_table(
                    ...
                    sorted_target_tids=self.sorted_tids
                    if self.target_turbine_indices != "all"
                    else "all",
        ...
        self.previous_control_signal = np.ones((3,)) * np.nan
    ...
    @staticmethod
    def _optimize_lookup_table(
        floris_config_path,
        uncertain,
        yaw_limits,
        parallel=False,
        optimization="scipy",
        sorted_target_tids="all",
        lut_path=None,
        generate_lut=True,
    ):
        if not generate_lut and lut_path is not None and os.path.exists(lut_path):
            ...
            if sorted_target_tids != "all":
                fi_lut.set(
                    layout_x=fi_lut.layout_x[sorted_target_tids],
            ...
            if uncertain:
                ...
                fi_lut.set(
                    ...
                    wd_stddevs=wd_stddevs_lut,
                    turbulence_intensities=[fi_lut.core.flow_field.turbulence_intensities[0]]
                    * len(ws_grid.flatten()),
                )
            else:
                fi_lut.set(
    ...
    def compute_controls(self):
        ...
        if (
            self._last_measured_time is not None
        ) and self._last_measured_time == self.measurements_dict["time"]:
            return
        ...
        if self.wf_source == "floris":
            ...
            current_ws_horz = self.measurements_dict["wind_speeds"] * np.sin(
                np.deg2rad(self.measurements_dict["wind_directions"] + 180.0)
            )
            current_ws_vert = self.measurements_dict["wind_speeds"] * np.cos(
                np.deg2rad(self.measurements_dict["wind_directions"] + 180.0)
            )
        else:
            ...
            current_ws_horz = current_row.select(
                [f"ws_horz_{tid}" for tid in self.tid2idx_mapping]
            ).to_numpy()[0, :]
            current_ws_vert = current_row.select(
        ...
        if len(self.measurements_dict["wind_directions"]) == 0 or np.all(
            np.isclose(self.measurements_dict["wind_directions"], 0)
        ):
            # yaw angles will be set to initial values
            if self.verbose:
                logging.info(
                    "Bad wind direction measurement received, reverting to previous measurement."
                )
        ...
        current_nd_cos[self.tgt_turbine_indices] = np.cos(
            np.deg2rad(self.measurements_dict["yaw_angles"])
        )
        current_nd_sin[self.tgt_turbine_indices] = np.sin(
            np.deg2rad(self.measurements_dict["yaw_angles"])
        )
        ...
        if self.wind_dir_use_filt or self.wind_mag_use_filt or self.wind_forecast:
            if self.historic_measurements is not None:
                self.historic_measurements = pl.concat(
                    [
                        self.historic_measurements,
                ...
                self.historic_measurements = current_measurements.select(
                    ["time"]
                    + self.ws_horz_cols
                    + self.ws_vert_cols
                    + self.nd_cos_cols
        ...
        reached_setpoints_cond = self.is_yawing & (
            current_yaw_setpoints == np.mod(self.previous_target_yaw_setpoints, 360)
        )
        if self.verbose and any(reached_setpoints_cond):
            logging.info(
                f"LUT Controller turbines {np.where(reached_setpoints_cond)[0]} have reached their target setpoint of {self.previous_target_yaw_setpoints[reached_setpoints_cond]} at time {self.current_time}"
            )
        ...
        if (
            ((self.current_time - self.init_time).total_seconds() % self.controller_dt) == 0.0
            ...
            if self.wind_forecast and self.wind_forecast.prediction_timedelta.total_seconds() > 0:
                if self.uncertain:
                    forecasted_wind_field = self.wind_forecast.predict_distr(
                        self.historic_measurements, self.current_time
                    )
                else:
                    forecasted_wind_field = self.wind_forecast.predict_point(
                        self.historic_measurements, self.current_time
                    )

                forecasted_wind_field = forecasted_wind_field.with_columns(
                    pl.col("time").cast(pl.Datetime(time_unit="ns")), cs.numeric().cast(pl.Float32)
                )
                ...
                if self.use_last_fcst_only:
                    ...
                    forecasted_wind_field = forecasted_wind_field.filter(
                        pl.col("time") == self.current_time + self.prediction_timedelta_stored
                    )
                ...
                single_forecasted_wind_field = forecasted_wind_field.filter(
                    pl.col("time") == self.current_time + self.wind_forecast.prediction_timedelta
                )
                ...
                fcst_cols = (
                    ...
                    + self.target_mean_ws_vert_cols
                    + (
                        (self.target_sd_ws_horz_cols + self.target_sd_ws_vert_cols)
                        if self.uncertain
                        else []
                    )
                )
                ...
                if self.forecasted_values is not None:
                    self.forecasted_values = (
                        ...
                        .agg(pl.all().last())
                    )  # predictions closer to time when made are probably more accurate
                else:
                    self.forecasted_values = forecasted_wind_field.select(fcst_cols)

            if not use_filt:
                wind = (
                    single_forecasted_wind_field
                    if use_wind_forecast
                    else current_measurements.select("time", cs.starts_with("ws_"))
                )
                ...
                if self.verbose:
                    if self.wind_forecast:
                        ...
                        logging.info(
                            f"unfiltered current wind directions = {self.current_wind_directions}"
                        )
                        logging.info(
                            f"unfiltered current wind directions = {self.current_wind_magnitudes}"
                        )
                ...
                if use_wind_forecast:
                    hist_meas = self.historic_measurements.select(
                        ["time"] + self.target_ws_horz_cols + self.target_ws_vert_cols
                    )
                    hist_meas = (
                        hist_meas.rename(
                            {
                                re.search("(?<=loc_)\\w+", new_col).group(0): new_col
                                for new_col in self.target_mean_ws_horz_cols
                                + self.target_mean_ws_vert_cols
                            }
                        )
                        if self.uncertain
                    ...
                    assume_persistence = (
                        hist_meas.select(
                            self.target_mean_ws_horz_cols + self.target_mean_ws_vert_cols
                        )
                        .slice(-1, 1)
                        .to_numpy()
                        == forecasted_wind_field.select(
                            self.target_mean_ws_horz_cols + self.target_mean_ws_vert_cols
                        ).to_numpy()
                    ).all()
                else:
                ...
                if assume_persistence:
                    ...
                    fcst_vals = fcst_vals.select(
                        pl.datetime_range(
                            start=last_historic_time,
                            end=last_forecasted_time,
                            interval=sim_timedelta,
                            time_unit="ns",
                            closed="right",
                        ).alias("time")
                    ).join(fcst_vals, on="time", how="left")

                    wind = pl.concat([hist_meas, fcst_vals], how="diagonal").select(
                        pl.col("time"), cs.numeric().interpolate(self.interpolation_method)
                    )

                    assert wind.select(
                        (pl.col("time").diff().slice(1) == sim_timedelta).all()
                    ).item() and (
                        wind.select(pl.col("time").last()).item()
                        == single_forecasted_wind_field.select(pl.col("time").last()).item()
                    ), (
                        "DataFrame passed to low pass filter must be continuous, with sampling time equal to simulation timestep, and must end on last forecasted value."
                    )
                ...
                if self.verbose:
                    if self.wind_forecast:
                        ...
                        logging.info(
                            f"unfiltered forecasted wind directions = {unfilt_wind_dirs[-1, :]}"
                        )
                        logging.info(
                            f"unfiltered forecasted wind magnitudes = {unfilt_wind_mags[-1, :]}"
                        )
                        ...
                        logging.info(
                            f"unfiltered current wind magnitudes = {self.current_wind_magnitudes}"
                        )
                ...
                if self.target_turbine_indices != "all" and (len(self.target_turbine_indices) == 2):
                    ...
                    wd = (
                        180.0 + np.rad2deg(np.arctan2(wind_u[-1, :].mean(), wind_v[-1, :].mean()))
                    ) % 360.0
                    layout_x_rot = (
                ...
                if self.wind_mag_use_filt:
                    ...
                    wind_u = np.array(
                        [
                            self._first_ord_filter(wind_u[:, i], self.wind_mag_lpf_alpha)
                            for i in range(len(self.sorted_tids))
                        ]
                    ).T  # [-int(self.controller_dt // self.simulation_dt), :]
                    wind_v = np.array(
                        [
                            self._first_ord_filter(wind_v[:, i], self.wind_mag_lpf_alpha)
                            for i in range(len(self.sorted_tids))
                        ]
                    ).T
                ...
                if self.verbose:
                    logging.info(
                        f"filtered {'forecasted' if self.wind_forecast else 'current'} wind directions = {wind_dirs}"
                    )
                    logging.info(
                        f"filtered {'forecasted' if self.wind_forecast else 'current'} wind magnitudes = {wind_mags}"
                    )

            if self.uncertain:
                ws_horz_stddevs = (
                    single_forecasted_wind_field.select(self.target_sd_ws_horz_cols)
                    .slice(0, 1)
                    .to_numpy()[0, :]
                )
                ws_vert_stddevs = (
                    single_forecasted_wind_field.select(self.target_sd_ws_vert_cols)
                    .slice(0, 1)
                    .to_numpy()[0, :]
                )
                forecasted_wind_norm = (
                    single_forecasted_wind_field.select(self.target_mean_ws_horz_cols)
                    .slice(0, 1)
                    .to_numpy()[0, :]
                    ** 2
                    + single_forecasted_wind_field.select(self.target_mean_ws_vert_cols)
                    .slice(0, 1)
                    .to_numpy()[0, :]
                    ** 2
                )
                ...
                wind_dir_stddevs = np.rad2deg(
                    ((c1 * ws_horz_stddevs) ** 2 + (c2 * ws_vert_stddevs) ** 2) ** 0.5
                )  # to degrees

                if self.verbose:
                    logging.info(
                        f"min wd_stdev = {min(wind_dir_stddevs)}, mean wd_stdev = {np.mean(wind_dir_stddevs)}, max wd_stdev = {max(wind_dir_stddevs)}"
                    )
            ...
            if self.target_turbine_indices == "all":
                wd_inp, wm_inp, wd_stddev_inp = (
                    wind_dirs.mean(),
                    wind_mags.mean(),
                    wind_dir_stddevs.mean() if self.uncertain else None,
                )
            else:
                # feeds wind from feed just upstream turbine to LUT, could also do mean
                ...
                if len(self.target_turbine_indices) == 2:
                    logging.info(
                        f"Using turbine idx {self.upstream_turbine_idx} as upstream turbine for input measurements to LUT at time {self.current_time}."
                    )
                    wd_inp, wm_inp, wd_stddev_inp = (
                        wind_dirs[self.upstream_turbine_idx],
                        wind_mags[self.upstream_turbine_idx],
                        wind_dir_stddevs[self.upstream_turbine_idx] if self.uncertain else None,
                    )
                else:
                    logging.info(
                        f"Using mean turbine measurements as input to LUT at time {self.current_time}."
                    )
                    wd_inp, wm_inp, wd_stddev_inp = (
                        wind_dirs.mean(),
                        wind_mags.mean(),
            ...
            if self.uncertain:
                target_yaw_offsets = self.wake_steering_interpolant(
                    ...
                    np.clip(
                        wd_stddev_inp,
                        self.wake_steering_interpolant.points[:, 2].min(),
                        self.wake_steering_interpolant.points[:, 2].max(),
                    ),
                )
            ...
            target_yaw_setpoints = np.mod(
                np.rint((wind_dirs - target_yaw_offsets) / self.yaw_increment) * self.yaw_increment,
                360.0,
            )
            ...
            abs_setpoint_change = np.vstack(
                [np.abs(setpoint_change), 360.0 - np.abs(setpoint_change)]
            )
            setpoint_change_idx = np.argmin(
                abs_setpoint_change, axis=0
            )  # if == 0, need to change within 360 deg, otherwise if == 1 faster to cross 360/0 boundary
            abs_setpoint_change = abs_setpoint_change[
                setpoint_change_idx, np.arange(self.n_turbines)
            ]
            ...
            dir_setpoint_change[setpoint_change_idx == 1] = -dir_setpoint_change[
                setpoint_change_idx == 1
            ]
            new_yaw_setpoints[is_target_changing] = (
                new_yaw_setpoints[is_target_changing]
                + dir_setpoint_change[is_target_changing] * abs_setpoint_change[is_target_changing]
            )
            self.is_yawing[is_target_changing] = True

            if self.verbose and any(is_target_changing):
                logging.info(
                    f"LUT Controller starting to yaw turbines {np.where(is_target_changing)[0]} from {current_yaw_setpoints[is_target_changing]} to {target_yaw_setpoints[is_target_changing]} in direction {dir_setpoint_change[is_target_changing]} at time {self.current_time}"
                )
        else:
        ...
        if self.verbose and any(reaching_setpoints_cond):
            ...
            logging.info(
                f"LUT Controller continuing to yaw turbines {np.where(reaching_setpoints_cond)[0]} from {current_yaw_setpoints[reaching_setpoints_cond]} to {self.previous_target_yaw_setpoints[reaching_setpoints_cond]} at time {self.current_time}"
            )
        ...
        new_yaw_setpoints[reaching_setpoints_cond] = self.previous_target_yaw_setpoints[
            reaching_setpoints_cond
        ].copy()
        ...
        self.previous_target_yaw_setpoints = (
            np.rint(new_yaw_setpoints / self.yaw_increment) * self.yaw_increment
        )

        lb, ub = (
            self.previous_yaw_setpoints - self.simulation_dt * self.yaw_rate,
            self.previous_yaw_setpoints + self.simulation_dt * self.yaw_rate,
        )
        ...
        self.previous_yaw_setpoints = (
            np.rint(constrained_yaw_setpoints / self.yaw_increment) * self.yaw_increment
        )
        ...
        if self.wind_forecast:
            ...
            if use_wind_forecast:
                ...
                newest_predictions = forecasted_wind_field.filter(
                    pl.col("time") == self.current_time + self.wind_forecast.prediction_timedelta
                ).select(
                    ["time"]
                    + self.target_mean_ws_horz_cols
